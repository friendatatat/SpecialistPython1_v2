...
staff_sorted = staff
for i in range(len(staff_sorted) - 1):
    for j in range(len(staff_sorted) - i - 1):
        if staff_sorted[i].get("salary") < staff_sorted[j].get("salary"):
            staff_sorted[j], staff_sorted[j + 1] = staff_sorted[j + 1], staff_sorted[j]
